...
function checkIncludedDependencies(path, root) {
	...
	dependencies?.forEach((dependency) => (recursiveDependency = recursiveDependency || checkIncludedDependencies(dependency, root)));
...
export default function (source, shader, options) {
	...
	return compress ? compressShader(loadChunks(source, shader, defaultExtension, warnDuplicatedImports, root)) : loadChunks(source, shader, defaultExtension, warnDuplicatedImports, root);
}
