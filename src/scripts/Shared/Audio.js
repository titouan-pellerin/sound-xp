...
export default class Audio {
	constructor() {
		...
		this.audioAmp = 5;
		this.audioIndex = 0.3;
		this.audioIndexStep = 0.06;
		this.isContrained = false;
		this.values = [];
		this.showPreview = true;
		this.easingAudioRangeValue = 0.48;
	...
	update = () => {
		...
		if (this.showPreview) {
			...
			this.ctx.fill();
			this.ctx.beginPath();
			this.ctx.rect(0, this.canvas.height - MARGIN - H_BEAT_BAR, x * (this.beatCutOff / this.lastBeat) * 0.8, H_BEAT_BAR);
			this.ctx.fillStyle = 'orange';
			this.ctx.fill();
			this.ctx.beginPath();
			this.ctx.rect(0, this.canvas.height - MARGIN - H_BEAT_BAR, x * (this.volume / this.lastBeat) * 0.8, H_BEAT_BAR);
